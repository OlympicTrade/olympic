...
class JsCompressor
{
    const SPECIAL_VAR_RX = '(\$+[a-zA-Z_]|_[a-zA-Z0-9$])[a-zA-Z0-9_$]*';
    ...
    function compress($code, $singleLine = true, $keepImportantComments = true, $specialVarRx = self::SPECIAL_VAR_RX)
    {
        $code = trim($code);
        if ('' === $code) return '';
                ...
                case '/':
                    if ('*' == $f[$i + 1]) {
                        ...
                        if (false !== strpos('-!%&;<=>~:^+|,(*?[{ ', $a)
                            ...
                            $a = $j;
                            $code .= $key;
                            while (isset($key[++$j - $a - 1])) $code[$j] = $key[$j - $a - 1];
                            --$j;
    ...
    protected function extractClosures($code)
    {
        ...
        while ($i) {
            ...
            switch (substr($f[$i - 2], -1)) {
                default:
                    if (false !== $c = strpos($f[$i - 1], ' ', 8)) break;
                case false:
                case "\n":
                case ';':
                case '{':
